...
class Game(object):
    ...
    def display_frame(self,screen):
        ...
        if self.game_over:
            ...
            text2= self.font.render(" Get Some benifits please",True,GREEN)
            # Put the text on the screen
            screen.blit(text,[120,20])

            for t in self.enemies.sprites():
                if t.rect.y==self.player.rect.y and (t.change_x*self.player.change_x)<0:
                   screen.blit(text2,[100,100])
                if t.rect.x==self.player.rect.x and (t.change_y*self.player.change_y)<0:
                   screen.blit(text2,[100,100])
            #
